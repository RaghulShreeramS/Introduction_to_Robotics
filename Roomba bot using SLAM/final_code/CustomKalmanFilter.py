...
class KalmanFilter(object):
    ...
    **Example**
    ...
    def update(self, z, R=None, H=None):
        ...
        self.y = self.clip_angle(self.y)
        # for i in range(2, len(self.y), 3):
        #     self.y[i] = (self.y[i] + np.pi) % (2 * np.pi) - np.pi
        ...
        # x = dot(dot(I_KH, self.P), I_KH.T) + dot(dot(self.K, R), self.K.T)
        # print("x after update:")
        # print(x)
        # print("What P should be after update:")
        # print(dot(dot(I_KH, self.P), I_KH.T) + dot(dot(self.K, R), self.K.T))
    ...
    def clip_angle(self, theta):
        change_flag = False
        new_theta = np.array(theta, copy=True)
        for i in range(len(new_theta) // 3):
            theta_i = new_theta[3*i+2]
            if  theta_i > np.pi:
                theta_i -= 2 * np.pi
                change_flag = True
            elif theta_i <= -np.pi:
                theta_i += 2 * np.pi
                change_flag = True
            new_theta[3*i+2] = theta_i
        # if change_flag == True:
        #     print("Scope angle: {} to {}".format(theta.T, new_theta.T))
        return new_theta
