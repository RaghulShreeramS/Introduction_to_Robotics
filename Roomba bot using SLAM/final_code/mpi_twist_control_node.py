...
class MegaPiControllerNode:
    def __init__(self, verbose=False, debug=False):
        ...
        # self.calibration = 80.0
        self.calibration = 50.0

    def twist_callback(self, twist_cmd):
        # desired_twist = self.calibration * np.array([[twist_cmd.linear.x], [twist_cmd.linear.y], [6*twist_cmd.angular.z]]) # 80 calibration
        desired_twist = self.calibration * np.array([[twist_cmd.linear.x], [twist_cmd.linear.y], [9*twist_cmd.angular.z]]) # 50 calibration hw 3
        # desired_twist = self.calibration * np.array([[twist_cmd.linear.x], [twist_cmd.linear.y], [6*twist_cmd.angular.z]]) # 50 calibration hw 4
        # desired_twist = self.calibration * np.array([[twist_cmd.linear.x], [twist_cmd.linear.y], [twist_cmd.angular.z]])
        print("Calibrated velocity: {}".format(desired_twist))
        ...
        # [FL, FR, RL, RR]
        print("Actual motor values: {}".format(result[:, 0]))
